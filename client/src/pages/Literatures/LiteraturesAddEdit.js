...
import { getAllLiteratures, submit } from "./literatures-api";

const LiteraturesAddEdit = () => {
    const {register, handleSubmit, formState: {errors, isDirty}, reset} = useForm()
    const params = useParams()
    const id = params.id
    const navigate = useNavigate()
    const isUpdate = params.id !=="new"
    const {send: getLiteratures} = useHttp(getAllLiteratures)
    const {send: literaturesSubmit} = useHttp(submit)
    ...
    const addLiteratureHandler = async (data) => {
        if(!isDirty) navigate(-1)
        try {
            await literaturesSubmit({data, isUpdate, id})
            getLiteratures()
            if(isUpdate) {
                toast.success("Adabiyot o'zgartirildi")
            }
            else toast.success("Adabiyot qo'shildi")
            navigate(-1)
        } catch (error) {
    ...
    const getById = async () => {
        const res = await axios.get(`http://localhost:8080/api/v1/literatures/${params.id}`)
        const byId = res.data.data.literatureById
        reset(byId)
    }
    return ( 
        <Layout title="Adabiyotni qo'shish/o'zgartirish">
            <form onSubmit={handleSubmit(addLiteratureHandler)}> 
                <input className={errors.name? "invalid": ""} type="text" placeholder="Nomi"
                {...register("name", {required: {value: true, message: "Adabiyot nomi kiritilmadi"}})}/>
                <br/>
...
export default LiteraturesAddEdit;